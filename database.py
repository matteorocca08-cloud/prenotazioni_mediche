...
def ottieni_visite_db():
    """Legge le visite da Supabase includendo il CF e restituisce dizionari completi."""
    try:
        # 1. Chiediamo esplicitamente a Supabase di includere il campo 'cf' e 'paziente'
        risposta = supabase.table("prenotazioni").select("paziente, cf, tipo_visita, data_ora, ticket").order("data_ora").execute()
        
        # 2. Restituiamo direttamente risposta.data (che è una lista di dizionari nativi)
        # Togliamo il vecchio ciclo for che tagliava fuori il codice fiscale!
...
def inserisci_prenotazione_db(nome, tipo, data, ora, cf):
